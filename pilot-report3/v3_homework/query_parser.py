...
오너 텔레봇 텍스트에서 과제 조회/확인 의도를 파싱합니다.
Preview only.
...
@dataclass
class RecallQuery:
    student_name: str          # 김주한
    date: str                  # YYYY-MM-DD  (source date)
    entry_type: str            # assigned_homework | homework_check | class_notes | ...
    intent: str                # recall_homework | register_homework | ...
    year_inferred: bool
    needs_review: bool
    raw_text: str
    parsed_at: str
...
def _parse_student(text: str, date_str: Optional[str]) -> Optional[str]:
    cleaned = re.sub(r'\d{4}[/-]\d{1,2}[/-]\d{1,2}', '', text)
    cleaned = re.sub(r'\b\d{1,2}[/-]\d{1,2}\b', '', cleaned)
    # 동사/조사 제거
    for w in ["불러와줘", "불러와", "불러줘", "등록해줘", "확인해줘",
...
def parse_query(text: str) -> RecallQuery:
    ...
    student_name = _parse_student(text, parsed_date)
    if not student_name:
        needs_review = True
        student_name = ""
